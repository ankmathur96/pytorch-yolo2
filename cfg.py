...
def load_conv(buf, start, conv_model):
    ...
    start = start + num_b
    conv_model.weight.data.copy_(torch.reshape(torch.from_numpy(buf[start:start+num_w]), (conv_model.weight.shape[0], conv_model.weight.shape[1], conv_model.weight.shape[2], conv_model.weight.shape[3])))
    start = start + num_w
    return start
